...
float4 fragBF(Varyings i) : SV_Target
{
	//return smpl(i.uv);
	...
	//return float4(stepDir, 0, 1); //これがゼロ？？？？？
	...
	//_BFRangeWeightが怪しそう？？？
	//return _BFRangeWeight[0] == 0.0 ? float4(1,0,0,1): float4(0,1,0,1);

	//return colorSum.a;
	//return colorSum.a == 1.0000 ? float4(1,0,0,1) : float4(0,1,0,1);
	//return colorSum / 0.3;
...
float4 fragFBF(Varyings i) : SV_Target
{
...
float4 fragTestBF(Varyings i) : SV_Target
{
	return smpl(i.uv);
}
