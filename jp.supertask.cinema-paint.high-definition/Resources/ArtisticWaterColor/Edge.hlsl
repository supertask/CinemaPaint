...
inline void sobel3(float2 uv, int index, inout float3 h, inout float3 v)
{
	float4 gradient = sobel3x3div4[index];
	float3 rgb = smpl(uv + (gradient * UV_SIZE)).rgb;
	h += rgb * gradient.z;
	v += rgb * gradient.w;
}

float4 fragSobel3(Varyings i) : SV_Target
{
	float3 h = 0.0, v = 0.0;
...
// Tangent Flow Map 
//////////////////////////////////////////////////////////////////////////////////////////////////
float4 fragTFM(Varyings i) : SV_Target
{
	//return smplSobelSimple(i.uv);
	//return smpl(_RT_SOBEL, i.uv);
	...
	// 勾配の大きさ（＝接線と注目画素の距離）
	float len = smplSobelSimple(i.uv).w * _SobelInvCarryDigit;
...
{
	tangent.xy = smplTFM(uv + offset).xy;
...
float4 fragLIC(Varyings i) : SV_Target
{
	float4 tfm = smplTFM(i.uv);
	float4 tangent = 0.0;
...
float4 fragOutline(Varyings i) : SV_Target
{
...
float4 fragTest(Varyings i) : SV_Target
{
