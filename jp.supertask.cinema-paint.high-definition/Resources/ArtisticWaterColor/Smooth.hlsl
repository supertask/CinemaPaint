...
float4 fragPosterize(Varyings i) : SV_Target
{
...
float4 fragSNN(Varyings i) : SV_Target
{
	// 前段でHSV空間にしている
	float4 hsvCenter = smplX(i.uv);
	float4 hsvSum = 0.0;
	...
	for (int y = 0; y <= _SNNRadius; ++y)
	{
		...
		for (int x = -_SNNRadius; x <= _SNNRadius; ++x)
		{
			...
			offset.x += UV_SIZE.x;
			float4 hsvNeighbor1 = smplX(i.uv + offset);
			float4 hsvNeighbor2 = smplX(i.uv - offset);
...
float4 fragGBlur(Varyings i) : SV_Target
{
	float4 colorSum = 0.0;
	for (int y = 0; y < _GBlurSize; ++y) {
		for (int x = 0; x < _GBlurSize; ++x) {
			...
			float4 colorNeighbor = smplX(i.uv + offset * UV_SIZE, _GBlurLOD);
...
float4 fragGBlur2(Varyings i) : SV_Target
{
	float4 colorSum = 0.0;
...
float4 fragSharpen(Varyings i) : SV_Target{
	float neighbor = unsharpBlur(i.uv);
	float4 center = smplX(i.uv, _SharpenLOD);
...
float4 fragUnsharpMask(Varyings i) : SV_Target{ return unsharpMask(i.uv); }
